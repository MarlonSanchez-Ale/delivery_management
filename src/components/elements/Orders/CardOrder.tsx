...
type Props = {
  data: Order;
  index: number,
  //onDragEnd: (event: { active: any; over: any }) => void; // Nueva prop para manejar el final del arrastre
}

/*const createSortableConfig = (id: string) => ({
  id: id,
  //onDragEnd,
});*/
...
export default function CardOrder(props: Props) {
  ...
  return (
      ...
      <div
        className='flex flex-col bg-white rounded-md shadow-md p-5 gap-8'
      ...
      >
        <div className='flex flex-col justify-start gap-3'>
          ...
          <Accordion placeholder="" open={open === index}>
            <AccordionHeader placeholder="" onClick={() => handleOpen(index)}>Products</AccordionHeader>
            <AccordionBody>
              <List placeholder="">
                {product && product.map(({ name, quantity }: OrderProduct, index) => (
                  <ListItem placeholder="" key={index}>
                    {name}
