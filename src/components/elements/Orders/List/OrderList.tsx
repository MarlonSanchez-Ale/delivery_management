...
import { reorderOrders, setFilter, deleteOrder } from '../../../../app/features/orders/OrderSlice';
                ...
                <div className='flex flex-col justify-center gap-5 p-5 my-5' >
                    <SortableContext items={itemsArray} strategy={verticalListSortingStrategy}>
                        {orders
                            ...
                            .map((order: Order) => (
                                <SortableItem key={order.idOrder} id={order.idOrder} onRemove={handleRemove} data={order} />
                            ))}
                    </SortableContext>
                    <DragOverlay>{activeId ? <CardOrder id={activeId} /> : null}</DragOverlay>
                </div>
            </div>
